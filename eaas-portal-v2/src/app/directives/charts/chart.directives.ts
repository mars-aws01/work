...
var textLimit = function(limit:number, labels: string): string{
  if(labels && labels.length){
    if(labels.length > limit){
      return labels.substring(0, limit) + '...';
    }
  }
  return labels;
}
...
Chart.defaults.global.legend.labels.generateLabels = function(chart) {
  var data = chart.data;
  var displayText = chart.options.legend.displayText==undefined? true:chart.options.legend.displayText;
  return Chart.helpers.isArray(data.datasets) ? data.datasets.map(function(dataset, i) {
    return {
      text: displayText ? dataset.label : '',
...
var doughnutGenerateLabels = function(chart) {
  var data = chart.data;
  var displayText = chart.options.legend.displayText==undefined? true:chart.options.legend.displayText;
  ...
  if (data.labels.length && data.datasets.length) {
    return data.labels.map(function(label, i) {
      ...
      return {
        text: displayText ? textLimit(chart.options.legend.textLimit, label) : '',
        fillStyle: fill,
        strokeStyle: stroke,
        lineWidth: bw,
...
Chart.defaults.doughnut.legend.labels.generateLabels = doughnutGenerateLabels;
Chart.defaults.pie.legend.labels.generateLabels = doughnutGenerateLabels;
...
export class BaseChartDirective implements OnDestroy, OnChanges, OnInit {
  ...
  @Input() public displayLegendTest:boolean;
  ...
  private updateChartDataAndLabels(newDataValues: number[] | any[], newLabels:string[] | any[]): void {
      ...
      (datasets && datasets.length)) {
        datasets = (this.datasets || datasets)
          .map((elm:number, index:number) => {
            ...
            if(useCustomerColor(this.chartType, this.datasets)){
              return newElm;
            }
...
function useCustomerColor(chartType:string, datasets:any) : boolean{
   if (chartType === 'pie' || chartType === 'doughnut') {
     if(datasets[0].backgroundColor && datasets[0].backgroundColor.length){
       return true;
     }
  }
