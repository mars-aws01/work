angular.module("negDateRangePicker",[])

.directive("negDateRangePicker", ["$compile", ($compile) ->
  # constants
  pickerTemplate = """
  <div ng-show="visible" class="angular-date-range-picker__picker" ng-click="handlePickerClick($event)">
    ...
    <div class="angular-date-range-picker__panel">
      ...
      <div class="angular-date-range-picker__buttons">
        <button ng-click="ok($event)" class="btn-success btn btn-sm">Apply</button>
        <button ng-click="hide($event)" class="btn-default btn btn-sm">cancel</button>
      </div>
    </div>
  </div>
  """
  ...
  <span class="angular-date-range-picker__input">
    ...
    <span ng-show="model!=null">{{ model.start.format("YYYY/MM/DD") }} - {{ model.end.format("YYYY/MM/DD") }}</span>
    <span ng-hide="model!=null" style='cursor:pointer;'>Select date range</span>
  </span>
  """
  scope:
  ...
  link: ($scope, element, attrs) ->
    ...
    _prepare = () ->
      ...
      $scope.range.by oneDayRange, (date) ->
        ...
        $scope.months[m].weeks[w][d] =
          date:     date
          selected: sel
          disabled: dis #|| date > moment(new Date()) todo:时间范围限制
    ...
    if attrs.pickerAlign && attrs.pickerAlign=="left"
      $(element).children("div").css("left","-1px")
