...
app.directive('ngFileSelect', [ '$fileUploader', function ($fileUploader) {
    ...
    return {
        link: function (scope, element, attributes) {
            //集成ACE Theme
            if (!attributes.url) {
              throw "system.directives.ngFileSelect -> error: required attribute 'url' is not found or empty.";
            }

            if (!scope.$eval(attributes.url)) {
              throw "system.directives.ngFileSelect -> error: '" + attributes.url + "' in attribute 'url' is not set or empty.";
            }
