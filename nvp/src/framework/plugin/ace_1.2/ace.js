...
ace.helper.removeStyle = function (elem, prop) {
    if ('removeProperty' in document.body.style)
        elem.style.removeProperty(prop);
    else
        elem.style[ace.helper.camelCase(prop)] = '';
}

ace.helper.hasClass = function (elem, className) {
...
ace.handle_side_menu = function ($) {
    var sidebar = $('.sidebar').eq(0);
    $(document).on('click' + '.ace.menu', '#menu-toggler', function () {
	...
	.on('click' + '.ace.menu', '.sidebar-collapse', function () {
	...
	.on('click' + '.ace.menu', '.sidebar-expand', function () {
...
ace.widget_boxes = function ($) {
    ...
    var Widget_Box = function (box, options) {
        ...
        this.reload = function () {
            ...
            $box.append('<div class="widget-box-overlay"><i class="' + ace.vars['icon'] + 'loading-icon icon icon-spinner icon-spin icon-2x white"></i></div>');
